...
var UserEvent = Backbone.Model.extend({
    ...
    url : function() {
        ...
        var url = window.location.protocol + "//" + getUrl(loc) + "events";
        return url;
...
var keywordsTable = new KeywordsHashTable({
    ...
    "concatenation" : "This connects the first and second parameters into a chain of the same type. Parentheses are mandatory around Concatenation expressions" ,
    "concept" : "A concept is a specification. Typically, it is a parameterized template and it includes the mathematical model used to abstract a type, the constraints, and specifications of operations to manipulate objects of that type." ,
    "Concept" : "A concept is a specification. Typically, it is a parameterized template and it includes the mathematical model used to abstract a type, the constraints, and specifications of operations to manipulate objects of that type." ,
    "conclusion" : "" ,
    "Confirm" : "" ,
    "conjunct" : "" ,
    ...
    "false" : "" ,
    "Family" : "Type Family is the family of types specified in a generic (parameterized) concept. A specific type results when the concept is instantiated." ,
    "finalization" : "" ,
    ...
    "of" : "" ,
    "oper" : "Operation is a user-oriented term for a method, and it begins the specification of an operation." ,
    "Oper" : "Operation is a user-oriented term for a method, and it begins the specification of an operation." ,
    "operation" : "Operation is a user-oriented term for a method, and it begins the specification of an operation." ,
    "Operation" : "Operation is a user-oriented term for a method, and it begins the specification of an operation." ,
    "or" : "" ,
    "otherwise" : "" ,
    "ponens" : "" ,
    ...
    submit.click(function(event){
        ...
        if(existingModel != null){
            ...
            if(existingModel != null){
                ...
                //var pathname = loc.pathname;
                //pathname = pathname.substring(0,pathname.lastIndexOf("/"));
                var url =  window.location.protocol + "//" + getUrl(loc) + "rename";
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {
...
function deleteUserComponent(openModel){
    ...
    //var pathname = loc.pathname;
    //pathname = pathname.substring(0,pathname.lastIndexOf("/"));
    var url = window.location.protocol + "//" + getUrl(loc) + "delete";
    $.ajax({
        type: "POST",
        url: url,
        data: {
...
function analyzeResults(resultJSON, component, waitGif){
    ...
    if(resultJSON.job == VCS){
        ...
        downloadButton.click(function(event){
            ...
            //pathname = pathname.substring(0,pathname.lastIndexOf("/"));
            //var url = "http://" + loc.host + (loc.pathname.length>1?loc.pathname+"/":loc.pathname) + "Components";
            var url = window.location.protocol + "//" + getUrl(loc)
